/**
 * LeadPilot scoring (client-side & server-side)
...
export type Term = { key: string; label: string; value: number };

export function buildTerms(f: LeadFeatures): Term[] {
...
export function channelUtilities(f: LeadFeatures): Record<Channel, number> {
...
export function softmax(logits: Record<Channel, number>): Record<Channel, number> {
