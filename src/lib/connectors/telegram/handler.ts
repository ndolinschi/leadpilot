...
export async function handleTelegramWebhook(
  ...
  if (!configuredToken && !secretHeader && !isTest) {
    return {
      ...
      setupInstructions: {
        ...
        step4: "For live testing in the browser demo, use the 'Send Test Webhook' action in the Connector Marketplace.",
      },
    };
  }
  ...
  const sender = rawMsg.from || rawMsg.chat || { id: 99999, first_name: "Demo", username: "telegram_user" };
  const senderName = [sender.first_name, (sender as { last_name?: string }).last_name]
    .filter(Boolean)
    .join(" ") || sender.username || `User ${sender.id}`;
  ...
  const uncalibratedLead: Lead = {
    id: leadId,
    name: senderName,
    company: sender.username ? `@${sender.username}` : "Telegram Direct Inquiry",
  ...
  return {
    ...
    statusCode: 200,
    message: isTest
      ? "Sample Telegram update verified and processed through connector handler"
    ...
    details: {
      ...
      mode: isTest ? "test-simulation" : "production-webhook",
    },
  };
}
